...
| G-15 | **Reports under-use the chrome component library.** Rendered reports were verbose monospace tables: `chrome.kpi_strip` called by 0/6 reports, `rdc-heatmap-cell` by 1, `section_card`/`rdc-copy-button` by 0, `.device-strip` CSS unused, no insight/"so what" layer, cryptic labels (`complexity * uses`, `d=/v=`). **c16** adds hero KPI strips + insight callouts (config `[report]` thresholds) + heatmap shading on ranked columns + a header device/provenance strip + label/unit fixes + icon empty-states (all reports). **c16b** adds the inline-SVG chart toolkit (`reports/charts.py`, ADR-33) + a flagship chart per report + the shader_hotlist column-diet `<details>`. **c16c** adds the remaining restructure (section-cards + sticky-h2 spread, copy-buttons, dashboard small-multiples + insight subtitles + cross-report nav, fill-or-hide, fuller a11y). | [c16](../commits/v02/c16_report_quality.md) (polish, done) · [c16b](../commits/v02/c16b_report_viz.md) (charts, done) · [c16c](../commits/v02/c16c_report_restructure.md) (restructure) | ☐ (c16 + c16b done; c16c open) |
