...
| G-15 | **Reports under-use the chrome component library.** Rendered reports were verbose monospace tables: `chrome.kpi_strip` called by 0/6 reports, `rdc-heatmap-cell` by 1, `section_card`/`rdc-copy-button` by 0, `.device-strip` CSS unused, no insight/"so what" layer, cryptic labels (`complexity * uses`, `d=/v=`). **c16** adds hero KPI strips + insight callouts (config `[report]` thresholds) + heatmap shading on ranked columns + a header device/provenance strip + label/unit fixes + icon empty-states (all reports). **c16b** adds the inline-SVG chart toolkit + a flagship chart per report + column-diet/section-cards/copy-buttons. | [c16](../commits/v02/c16_report_quality.md) (polish) · [c16b](../commits/v02/c16b_report_viz.md) (charts/restructure) | ☐ (c16 done; c16b open) |
